					...
					<ul className="font-medium flex flex-col p-4 md:p-0 mt-4 border rounded-lg md:flex-row md:space-x-8 md:mt-0 md:border-0  bg-gray-800 md:bg-transparent border-gray-700">
						{
							navLists.map((navList, index) => (
								<li key={navList.link}>
									<Link
										to={navList.link}
										className={cn("block py-2 pl-3 pr-4 rounded md:hover:bg-transparent md:border-0 md:p-0 text-white md:hover:text-blue-500 hover:bg-gray-700 hover:text-white cursor-pointer relative", activeLink === `${navList.link}` ? "underline" : "")}
										activeClass="active"
										spy={true}
										smooth={true}
										offset={-96}
